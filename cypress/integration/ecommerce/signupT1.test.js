...
describe("buy a product and check the quantity inside the cart", () => {
  ...
  it("click on a category and add multiple items to the cart", () => {
    ...
    cy.get(
      'a[title="View my shopping cart"] span[class="ajax_cart_quantity"]'
    ).should("have.text", "2");
  ...
  it("login and check if the item details are correct", function () {
    ...
    //assertion landded on right page
    //check element for each actions
    //we are carrying out an acction and checking the result of that action
    //chropath tool to grab an element
    //cypress doc read uniqueness
    //nevr do a flakey test, cypress time out do reading
    //do more assertion
    //look into changing the fixture folder
    // try to do more in headless
    // re read the cypress
  });
});
